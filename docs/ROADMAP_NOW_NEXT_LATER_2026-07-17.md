...
**All 10 items below are done as of 2026-07-17.** `Later` is the current priority list.
...
8. ~~Add a lightweight mobile diagnostics screen showing base URL, auth state, health, tunnel, and version.~~ Done 2026-07-17: new `app/diagnostics.tsx` (a modal route, reached via a "View Diagnostics" button in Settings' About section) shows exactly the requested fields — base URL, auth state (token set / no token / rejected), server reachability + uptime + active sessions + version (via item 10's `useServerStatus`), tunnel status + URL, and app version — plus a "Copy Diagnostics to Clipboard" action so a user can paste a snapshot into a support message without manually typing out five different values. Reuses `useServerStatus`/`useRuntimeConfig` rather than fetching anything itself, so it stays in sync with Settings' own view of server state for free.
